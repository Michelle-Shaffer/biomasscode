...
# calamus spp, but used calamus penna code
### For other years ###
#Determine length binning in Species length binnning code file
##Calculate weights:
#Convert SL into cm:
table$cm <- table$AvgLength/10
table$TL <- (table$cm*1.179) #add this info to summary table
table$FL <- (table$TL/1.082) #add this info to summary table
...
table
table$AvgWeight <- 0.07640*(table$FL^2.666)
table
